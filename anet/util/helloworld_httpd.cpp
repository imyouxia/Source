...
void doProcess(unsigned int port, unsigned int num) {
    ...
    ANET_LOG(INFO, "*****************************************************"); 
    ANET_LOG(INFO, "THREAD_ID TOT_TIME(ms) RUN_TIME(ms) WAIT_TIME(ms) TASKS_PROCESSED");
    for (long i=0; i<num; i++) {
        ANET_LOG(INFO, "%8ld %12.2f %12.2f %13.2f %12ld", runnables[i].m_id, 
